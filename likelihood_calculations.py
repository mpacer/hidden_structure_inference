...
def p_graph_given_d(graphs,options):
    ...
    for i,graph in enumerate(graphs):
        loglikelihood[i] = parameters_monte_carlo_loglik(graph,param_sample_size,options=options)
        # prints progres at roughly every 10% of progress
        if i in [int(np.floor(j*len(graphs))) for j in np.arange(0,1,.1)]:
            sys.stdout.write("{:.2%} ".format(i/len(graphs)))
            sys.stdout.flush()
    
    sparsity = options["sparsity"]
    logposterior = logposterior_from_loglik_logsparseprior(graphs,loglikelihood,sparsity)
#     import ipdb; ipdb.set_trace()
    return graphs,np.exp(logposterior),logposterior,loglikelihood,options
...
def sparse_graphset_prior(graphs,sparsity=.5):
    ...
    unnormed_p = [sparse_1graph_prior_unnormed(graph,max_edges,sparsity=sparsity) for graph in graphs]
    return unnormed_p/sum(unnormed_p)

def log_sparse_graphset_prior(graphs,sparsity=.5):
    max_edges = np.max([len(x.edges()) for x in graphs])
    unnormed_logp = [log_sparse_1graph_prior_unnormed(graph,max_edges,sparsity=sparsity) for graph in graphs]
    return unnormed_logp - mdp_logsumexp(unnormed_logp)
                                                                   
                                                                   
def parameters_monte_carlo_loglik(graph, param_sample_size, options = None):
    ...
    param_samples = _helper_iter_param_sampler(gs_in,gp_in,gs_out,gp_out,param_sample_size,options)
    
    return logmeanexp(np.fromiter(param_samples,dtype=np.float,count=param_sample_size))

def _helper_iter_param_sampler(gs_in,gp_in,gs_out,gp_out,param_sample_size,options):
    ...
    for i in range(param_sample_size):
        ...
        yield aux_data_monte_carlo_loglik(gs_in,gp_in,gs_out,gp_out,stigma_sample_size,options=options)
...
def gen_iter_simulations(gs_in,gp_in,M):
    # builds a simulation object and then samples returning an M lengthed generator
    inner_simul = InnerGraphSimulation(gs_in,gp_in)
    return inner_simul.sample_iter(M)

def aux_data_monte_carlo_loglik(gs_in, gp_in, gs_out, gp_out, stigma_sample_size, options=None):
    stigma_sample_size = options["stigma_sample_size"]
#     inner_samp = gen_simulations(gs_in, gp_in, stigma_sample_size)
    inner_samp = gen_iter_simulations(gs_in, gp_in, stigma_sample_size)
    ...
    sim_loglike = (cross_entropy_loglik(data_sets, data_probs, num_data_samps, stigma, obs_dict) for stigma in inner_samp)
...
def cross_entropy_loglik(data_sets,data_probs, k , aux_data, obs_dict):
    ...
    return np.sum([data_probs[i]*k*multi_edge_loglik(obs_data, aux_data, obs_dict) for i,obs_data in enumerate(data_sets)])
